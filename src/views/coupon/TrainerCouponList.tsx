...
import { PutTrainerCouponRequest } from "@/apis/coupon/put-trainer-coupon.api";
...
function TrainerCouponList() {
  ...
  useEffect(() => {
    const fetchCoupons = async () => {
      ...
      if (!token) {
        alert("Access token 없음. 인증된 사용자만 조회 가능합니다.");
        return;
      }
  ...
  const handleConfirm = async () => {
    ...
    const response = await PutTrainerCouponRequest(dto, selectedCouponId, token);
    if (response.code === "SU") {
  ...
  return (
    <div>
      ...
      {showModal && (
        <div css={t.trainerCouponModalOverlay}>
          <div css={t.trainerCouponModalContent}>
            <h3>쿠폰 사용 날짜 입력</h3>
            <input
              type="date"
              value={usedDate}
            ...
            />
            <div css={t.trainerCouponModalButtons}>
