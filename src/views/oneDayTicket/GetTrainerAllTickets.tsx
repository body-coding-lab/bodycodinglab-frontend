...
const GetTrainerAllTickets = () => {
  ...
  const [filter, setFilter] = useState<'ALL' | 'ISSUED' | 'USED' | 'CANCEL'>('ALL');
  ...
  if (tickets.length === 0) {
    return <p css={s.emptyTicketMessage}>발급된 체험권이 없습니다.</p>;
  }

  return (
    <section>
      <h2 css={s.mainTitle}>전체 체험권 목록</h2>

      <div css={s.filterButtonContainer}>
        <div css={s.filterButtonsLeft}>
          <button css={s.filterButton(filter === 'ALL')} onClick={() => setFilter('ALL')}>전체</button>
          <button css={s.filterButton(filter === 'ISSUED')} onClick={() => setFilter('ISSUED')}>발급</button>
          <button css={s.filterButton(filter === 'USED')} onClick={() => setFilter('USED')}>사용</button>
          <button css={s.filterButton(filter === 'CANCEL')} onClick={() => setFilter('CANCEL')}>취소</button>
        </div>
      </div>
        ...
        {paginatedTickets.map(ticket => (
          <article key={ticket.id} css={s.ticketCard}>
            ...
            <div css={s.ticketBottom}>
              ...
              {ticket.usedAt && <div css={s.usedButton}>사용일자: {ticket.usedAt}</div>}
              {ticket.canceledAt && <div css={s.cancelButton}>취소일자: {ticket.canceledAt}</div>}
            </div>
          </article>
        ))}
      </div>
      ...
      {totalPages > 1 && (
        <div css={s.paginationWrapper}>
          {Array.from({ length: totalPages }, (_, index) => (
            <button
              key={index}
              css={index + 1 === currentPage ? s.activePaginationButton : s.paginationButton}
