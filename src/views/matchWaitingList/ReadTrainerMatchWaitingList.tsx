...
import * as t from "./TrainerMatchWaitingListStyle";
...
import { GetTrainerMatchWaitingListRequest } from "@/apis/matchWaitingList/get-traienr-matchWaitingList.api";
import { PutTrainerMatchWaitingListApproveRequest } from "@/apis/matchWaitingList/put-trainer-matchWaitingList-approve.api";
import { PutMemberMatchWaitingListRejectRequest } from "@/apis/matchWaitingList/put-trainer-matchWaitingList-reject.api";

function ReadTrainerMatchWaitingList() {
  const [cookies, setCookies] = useCookies(["accessToken"]);
  const [memberDatas, setMemberDatas] = useState<
  GetTrainerMatchWaitingListResponseDto[] | undefined
  >(undefined);
  ...
  const loadingMemberData = async () => {
    ...
    try {
      const response = await GetTrainerMatchWaitingListRequest(token);
  ...
  const matchApproveButton = async (matchWaitingListId: number) => {
    ...
    const response = await PutTrainerMatchWaitingListApproveRequest(
      matchWaitingListId,
    ...
    const response = await PutMemberMatchWaitingListRejectRequest(
      matchWaitingListId,
      {
  ...
  return (
    <div>
      ...
      {showRejectModal && selectedMatchId !== null && (
        <div css={t.trainerRejectModalOverlay}>
          <div css={t.trainerRejectModalContent}>
            <h3>거절 사유 입력</h3>
            <textarea
              rows={5}
            ...
            />
            <div css={t.trainerRejectModalButtons}>
              <button
