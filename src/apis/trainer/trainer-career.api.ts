...
import { DELETE_ALL_TRAINER_CAREER, DELETE_TRAINER_CAREER, GET_TRAINER_CAREER, GET_TRAINER_CAREER_RECENT, POST_TRAINER_CAREER, UPDATE_TRAINER_CAREER } from "../constants";
...
export const updateCareer = async (careerId: number, dto: TrainerCareerRequestDto, accessToken: string): Promise<ResponseDto<TrainerCareerResponseDto>> => {
  try {
    const response = await axiosInstance.put(UPDATE_TRAINER_CAREER(careerId), dto, bearerAuthorization(accessToken));
    return responseSuccessHandler(response);
  } catch (error) {
    return responseErrorHandler(error as AxiosError<ResponseDto>);
  }
